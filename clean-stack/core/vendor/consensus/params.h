...
struct Params {
     ...
     * DEFAULT 1 = the MatMul v4.7 Epoch-A shape with ExactReplay as the sole
     * authority, byte-identical to the pre-datacenter episode behavior.
     * Profile 2 is retained for the later proof-authoritative datacenter epoch.
     * Its sublinear Freivalds sampled verifier remains only a relay/precheck and
     * never final consensus authority; if profile 2 is selected before durable
     * Stage-3 authority exists, it still falls through to ExactReplay (safe but
     ...
     *  182 bytes. Public-network construction therefore requires this field to
     *  remain disabled. MatMul v4.7 uses the Poseidon2 rcadmit policy sidecar
     *  and zero pre-ExactReplay chainwork credit instead. The dormant helper
     *  survives only for isolated regtest/history tests. */
    ...
    // Miner overlay (not a BTX struct member). Pool autolatch / operator pin.
    // The BTX 0.34 instance field nMatMulRCHeight is the ABI source of truth
    // for SelectParams(); these statics let classic pool mode latch without
    // a compiled height, and survive if chainparams still say INT32_MAX.
    ...
    {
        if (IsMatMulV4Active(height)
            && nMatMulRCHeight != std::numeric_limits<int32_t>::max()
            && height >= nMatMulRCHeight) {
            return true;
        }
        const int32_t latched = RCActivationHeight();
        return fMatMulPOW &&
            height >= 0 &&
            latched != std::numeric_limits<int32_t>::max() &&
            height >= latched;
    }
    ...
    /** True iff the withdrawn HeaderPoW helper has a configured discount.
     *  Public networks categorically require false because the grind nonce is
     *  absent from the fixed 182-byte wire. Callers additionally height-gate
     *  isolated regtest/history coverage. */
    bool IsMatMulHeaderPoWEnabled() const
    {
        return nMatMulHeaderPoWDiscountBits != std::numeric_limits<uint32_t>::max();
    }
    /** Exact public-network tuple for the v4.7 Epoch-A cutover.
     *
     *  Epoch A is one atomic v3 -> Profile-1 ExactReplay fork. The legacy
     *  ENC-BMX4C and RC gates share the same height so no digest-only v4
     *  interval exists before ExactReplay authority. DRLT, coupled RC,
     *  HeaderPoW, toy dimensions, and later proof/profile transitions remain
    ...
    bool IsMatMulV47EpochAActivationTuple() const
    {
        ...
        return nMatMulV4Height != disabled &&
               nMatMulBMX4CHeight == nMatMulV4Height &&
               nMatMulRCHeight == nMatMulV4Height &&
               nMatMulDRLTHeight == disabled &&
               nMatMulRCCoupledHeight == disabled &&
               nMatMulRCProfile == 1 &&
               !fMatMulRCUseToyDims &&
               !fMatMulRCCoupledUseToyDims &&
               !fMatMulLTSealAsPoW &&
               !IsMatMulHeaderPoWEnabled() &&
               nMatMulV4AsertRescaleNum == nMatMulV4AsertRescaleDen &&
               nMatMulBMX4CAsertRescaleNum == nMatMulBMX4CAsertRescaleDen;
    }
