#!/usr/bin/env bash
# build-clean-rel.sh -- build a RELEASE btxluxminer straight from clean-stack (#5, Path B).
...
#   docker build -t btxlux-build:deps -f clean-stack/Dockerfile.pathb-deps-cm4 clean-stack
set -euo pipefail
S="${S:-$HOME/git/matador-src/clean-stack}"
BTX="${BTX:-$HOME/git/btx-stock/build}"
IMG="${IMG:-btxlux-build:deps}"   # CUDA 13.3 + CMake 4.3.2 (see NOTE above)
...
docker run --rm $CPUARG -e "ARCH=$ARCH" -e "VER=$VER" -e "JOBS=${CPUS:-$(nproc)}" \
  ...
  cmake -B build-rel -S core -DMATADOR_ENABLE_CUDA=ON -DMATADOR_CUDA_ARCH="$ARCH" \
        ...
        \
         -DBTX_ARCHIVE_DIR=/btx \
          -DCMAKE_CXX_FLAGS="-fopenmp" \
          -DCMAKE_CXX_STANDARD_LIBRARIES="-L/usr/local/cuda/targets/x86_64-linux/lib -lcublasLt -lcublas -lcudart -lgomp" \
        $CUTLASS_ARG
  cmake --build build-rel --target rc_probe btxluxminer --parallel "$JOBS"
  echo "[byte-exact gate] expect episode golden 5b1bff3c..."; ./build-rel/rc_probe
  strip --strip-unneeded build-rel/btxluxminer || true
  mkdir -p dist
  cp build-rel/btxluxminer dist/btxluxminer
  ( cd dist && sha256sum btxluxminer | tee btxluxminer.sha256 )
'
echo "built: $S/dist/btxluxminer ($VER)"
